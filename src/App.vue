...
const diceOne = useDice()
const diceTwo = useDice()
const diceThree = useDice()
const diceFour = useDice()
const diceFive = useDice()

const dices = [diceOne, diceTwo, diceThree, diceFour, diceFive]

const rollDice = () => {
	const tempDices: number[] = []
	dices.forEach(item => {
		item.roll()
		tempDices.push(item.face.value)
	})
	console.log(game.diceFaces.value);
	game.updateCategories(tempDices)
		...
		<main class="flex flex-col gap-6">
			<ul class="flex gap-4">
				<li v-for="(dice, index) in dices" :key="index" @click="dice.hold()"
					class="flex flex-col justify-center text-center bg-stone-200 text-black border-4 border-black-500 rounded size-16 cursor-pointer text-xl font-bold transition-colors duration-300"
					:class="{ 'border-red-700': dice.isHold.value }">
					{{ dice.face }}
				</li>
			</ul>
			<CategorySection />
			<button @click="rollDice()" class="size-20 cursor-pointer text-red-700">
